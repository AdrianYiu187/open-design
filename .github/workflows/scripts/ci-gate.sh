...
function isSelected(atom) {
  if (provider === "runner") return atom.name !== "nix";
  if (provider === "hosted" && mode === "nix") return atom.name === "nix";
  if (provider === "hosted" && mode === "full") return true;
  throw new Error(`unsupported provider/mode: ${provider}/${mode}`);
}

function unavailableFor(atom) {
  if (provider === "runner" && atom.name === "nix") {
    return {
      atom: atom.name,
